...
with open("Dia 4/input.txt") as file:
    while True:
        ...
        if ((int(pair1[0]) <= int(pair2[0]) and int(pair1[1]) >= int(pair2[1]))
         ...
         or (int(pair1[1]) >= int(pair2[0]) and int(pair1[1]) <= int(pair2[1]))
         or (int(pair1[0]) >= int(pair2[1]) and int(pair1[0]) <= int(pair2[1]))):
            fulloverlaps += 1
        # es una chapuza pero estoy hasta los cojones de esta pregunta la vd
